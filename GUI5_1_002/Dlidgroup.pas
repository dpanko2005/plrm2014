...
uses
  Dlidusage, Ulid,    DlidUsage1;

const
  ColLabels: array[0..4] of String =
    ('Control Name', 'LID Type', '% of Area', '% From Imperv', 'Report File');
...
begin
  ...
  begin
    ...
    begin
      ...
      for J := 1 to 8 do Data[J] := LidUnit.Data[J-1];
...
begin
  // Set location of error message dialog
  Pclient.X := Width div 4;
  Pclient.Y := Height div 4;
  ...
  if TotalArea > 100.01 then
    MessageDlgPos('Total LID area exceeds that of subcatchment.',
      mtError, [mbOK], 0, Pscreen.X, Pscreen.Y)
  else if TotalImperv > 100.01 then
    MessageDlgPos('Percent of impervious area treated exceeds 100.',
      mtError, [mbOK], 0, Pscreen.X, Pscreen.Y)
...
begin
  ...
  begin
    ...
    // Place percent impervious treated & report file name in columns 3-4
    Cells[3, I] := Data[5];
    Cells[4, I] := Data[7];
  end;
end;
