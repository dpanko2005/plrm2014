...
begin
  ...
  if (I < 0)
  then MessageDlg(S + ' is not a registered tool.', mtWarning, [mbOK], 0)
  else
  begin
    ...
    if Tool.DisableSWMM then Application.Minimize;
    R := Uutils.WinExecAndWait(CmdLine, DirName, SW_SHOWNORMAL,
                               Tool.DisableSWMM);
    if R < 0 then MessageDlg('Could not launch ' + ToolList[I], mtWarning,
                             [mbOK], 0);
    if Tool.DisableSWMM then Application.Restore;
