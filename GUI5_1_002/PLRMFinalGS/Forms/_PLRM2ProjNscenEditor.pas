...
uses
  ...
  Dialogs, jpeg, ExtCtrls, StdCtrls, Buttons, DB, ADODB, Grids, DBGrids, GSDataAccess,
  xmldom, XMLIntf, msxmldom, XMLDoc,  FileCtrl, GSUtils, GSPLRM, ComCtrls, GSTypes, GSNodes, ShellApi;
...
var
  pdfpath :PAnsiChar;
begin
  pdfPath := PAnsiChar(AnsiString(extractFilePath(Application.ExeName) + 'Grid_Map.pdf')) ;
...
var
  ...
  prjIdx : Integer;
begin
   if (tbxMetGrid.Text = '') OR  (StrToInt(tbxMetGrid.Text)<1) OR (StrToInt(tbxMetGrid.Text)>1223) then
    begin
    ...
    begin
        ...
        begin
          ShowMessage('The name you have provided is already in use. Please try another name');
          exit;
        end;
    end;
     ...
     if DirectoryExists(PLRMObj.projFolder) = False then //need to create both folders
...
begin
  dlgOpenFile.Filter := 'Access Database files (*.accdb)|(*.accdb)|Access Database files (*.accdbe)|(*.accdbe)|All files (*.*)|*.*';
...
begin
  ...
  begin
    if (PLRMTree.checkScnNameUsed(tbxProjName.Text, tbxScenName.Text) = true) then
    ShowMessage('The name you have provided is already in use. Please try another name');
  end;
end;
...
var
  ...
  begin
      ...
      begin
        if (PLRMTree.checkScnNameUsed(tbxProjName.Text, tbxScenName.Text) = true) then
        begin
          ShowMessage('The Scenario name you have provided is already in use. Please try another name');
          exit;
        end;
      end;
    ...
    begin
      ...
      if FileExists(PLRMObj.scenarioXMLFilePath) = True then //update
        ...
        PLRMObj.plrmToXML;
      MainForm.Caption := Fmain.TXT_MAIN_CAPTION + '[Project Name: ' + PLRMObj.projUserName +  '] [Scenario Name: ' + PLRMObj.scenarioName + ' ]';
      //PLRM Edit Jan 2010 edit added to track whether user working with scenario see #233
