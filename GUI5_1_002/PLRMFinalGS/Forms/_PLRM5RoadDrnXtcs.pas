...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, jpeg, Grids, GSUtils, GSTypes, GSPLRM,
  GSCatchments, UProject,
...
type
  TPLRMDrainageConditions = class(TForm)
    ...
    procedure sgGenDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect;
      State: TGridDrawState);
    procedure sgGenParcelDrawCell(Sender: TObject; ACol, ARow: Integer;
      Rect: TRect; State: TGridDrawState);
    procedure sgGenRdDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect;
      State: TGridDrawState);
    procedure sgRoadGenSelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
    procedure sgParcelGenSelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
    procedure sgGenRdSelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
    ...
    procedure sgGenSetEditText(Sender: TObject; ACol, ARow: Integer;
      const Value: string);
    procedure sgVegDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect;
      State: TGridDrawState);
    procedure checkAndSelectCbxItm(var cbx: TComboBox;
      scheme: TPLRMHydPropsScheme; schms: TStringList);
...
begin
  GSPLRM.PLRMObj.currentCatchment.secRdDrng := GSUtils.copyGridContents(0, 0,
    sgSecRds);
  GSPLRM.PLRMObj.currentCatchment.primRdDrng := GSUtils.copyGridContents(0, 0,
    sgPrimRds);
  GSPLRM.PLRMObj.currentCatchment.sfrDrng := GSUtils.copyGridContents(0,
    0, sgSFR);
  GSPLRM.PLRMObj.currentCatchment.mfrDrng := GSUtils.copyGridContents(0,
    0, sgMFR);
  GSPLRM.PLRMObj.currentCatchment.cicuDrng := GSUtils.copyGridContents(0,
    0, sgCICU);
  GSPLRM.PLRMObj.currentCatchment.vegTDrng := GSUtils.copyGridContents(0,
    0, sgVeg);
  GSPLRM.PLRMObj.currentCatchment.othrDrng := GSUtils.copyGridContents(0,
    0, sgOthr);
...
begin
  PLRMObj.currentCatchment.mfrSchms[1] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.mfrSchms[1], false, 1);
end;

// SFR schemes Edit
procedure TPLRMDrainageConditions.btnEditSFRInfClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.sfrSchms[1] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.sfrSchms[1], false, 1);
end;

// CICU Schemes Edit
procedure TPLRMDrainageConditions.btnEditCICUInfClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.cicuSchms[1] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.cicuSchms[1], false, 1);
end;

// Othr scheme edit
procedure TPLRMDrainageConditions.btnEditOthrOutClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.othrSchms[0] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.othrSchms[0], false, 0);
end;
{$ENDREGION}
{$REGION 'Roads' }
...
begin
  PLRMObj.currentCatchment.secRdSchms[1] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.secRdSchms[1], true, 1);
end;

procedure TPLRMDrainageConditions.btnRdSecDspEditClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.secRdSchms[2] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.secRdSchms[2], true, 2);
end;
{$ENDREGION}
...
begin
  PLRMObj.currentCatchment.primRdSchms[1] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.primRdSchms[1], true, 1);
end;

procedure TPLRMDrainageConditions.btnRdPrimDspEditClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.primRdSchms[2] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.primRdSchms[2], true, 2);
end;

procedure TPLRMDrainageConditions.btnRdPrimOutEditClick(Sender: TObject);
begin
  PLRMObj.currentCatchment.primRdSchms[0] :=
    getSDrngXtsScheme(PLRMObj.currentCatchment.primRdSchms[0], true, 0);
end;
{$ENDREGION}
{$ENDREGION}
...
begin
  ...
  begin
    ShowMessage
      ('Please select another catchment or go back and provide soils data for current catchment');
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((J = 1) and (I > 1)) then
          sgs[I].Cells[3, J] := '50';
        // hardcoded default dcia for non hsc catchs

        // default ksat vals  8/14/09 apply ksat reduction factors from database
        sgs[I].Cells[4, J] := FormatFloat('0.##',
          (StrToFloat(soilsInfData[0, 1]) * StrToFloat(kSatMultplrs[0][I])));
        // default slopes
        ...
        sgs[I].Cells[5, J] := hydProps[0][2];
        // note that row 0 and 1 are mannings values in hydProps
    ...
    sgSecRds.Cells[4, 2] := FormatFloat('0.##', StrToFloat(soilsInfData[0, 1]) *
      StrToFloat(kSatMultplrs[0][1]));
    // 8/14/09 apply ksat reduction factors from database
    // default pervious area depression storage
    sgSecRds.Cells[5, 2] := hydProps[0][2];
    // note that row 0 and 1 are mannings values in hydProps
    // default impervious area depression storage
    ...
    sgPrimRds.Cells[4, 2] := FormatFloat('0.##', StrToFloat(soilsInfData[0, 1])
      * StrToFloat(kSatMultplrs[0][0]));
    // default pervious area depression storage
    sgPrimRds.Cells[5, 2] := hydProps[0][2];
    // note that row 0 and 1 are mannings values in hydProps
    // default impervious area depression storage
    ...
    begin
      curAcVals[I] := PLRMObj.getCurCatchLuseProp(GSUtils.frmsLuses[I], 3,
        hasLuse[I]);
      curImpPrctVals[I] := PLRMObj.getCurCatchLuseProp(GSUtils.frmsLuses[I], 2,
        hasLuse[I]);
    end;
    ...
    begin
      ...
      sgPrimRds.Cells[2, 2] := FormatFloat('0.##',
        curImpPrctVals[0] * curAcVals[0] / 100);
    end;
    // secondary road areas
    if hasLuse[1] = true then
    begin
      sgSecRds.Cells[1, 2] := FormatFloat('0.##', curAcVals[1]);
      sgSecRds.Cells[2, 2] := FormatFloat('0.##',
        curImpPrctVals[1] * curAcVals[1] / 100);
    end;
    ...
    begin
      ...
      begin
        ...
        sgs[J].Cells[1, 1] := FormatFloat('0.##',
          ((100 - prcnt) * curAcVals[J] / 100));
        sgs[J].Cells[2, 0] := FormatFloat('0.##',
          (prcnt * curImpPrctVals[J] * curAcVals[J] / 100 / 100));
        sgs[J].Cells[2, 1] := FormatFloat('0.##',
    ...
    prcnt := StrToFloat(bmpImpl[J - 2, 2]);
    sgVeg.Cells[0, 0] := '100';
    sgVeg.Cells[1, 0] := FormatFloat('0.##',
      ((100 - prcnt) * curAcVals[J] / 100));
    sgVeg.Cells[2, 0] := FormatFloat('0.##',
      ((100 - prcnt) * curImpPrctVals[J] * curAcVals[J] / 100 / 100));
    sgVeg.Cells[3, 0] := '50';
    ...
    sgOthr.Cells[2, 0] := FormatFloat('0.##',
      (PLRMObj.currentCatchment.othrPrcntImpv));
    sgOthr.Cells[3, 0] := '50';
    // lblCatchArea.Caption := 'Selected Catchment Area is: ' + FloatToStr(area) + 'ac';

    lblPrrHdr.Caption := 'Primary Roads ( ' + FormatFloat('0.##',
      StrToFloat(sgPrimRds.Cells[1, 2])) + ' acres)';
    lblSerHdr.Caption := 'Secondary Roads ( ' + FormatFloat('0.##',
      StrToFloat(sgSecRds.Cells[1, 2])) + ' acres)';
    lblSfrHdr.Caption := 'Single Family Residential ( ' +
      FormatFloat('0.##', StrToFloat(sgSFR.Cells[1, 1])) + ' acres)';
    lblMfrHdr.Caption := 'Multi-Family Residential ( ' +
      FormatFloat('0.##', StrToFloat(sgMFR.Cells[1, 1])) + ' acres)';
    lblCicHdr.Caption := 'CICU ( ' + FormatFloat('0.##',
      StrToFloat(sgCICU.Cells[1, 1])) + ' acres)';
    lblVegHdr.Caption := 'Vegetated Turf ( ' + FormatFloat('0.##',
      StrToFloat(sgVeg.Cells[1, 0])) + ' acres)';
    lblOthHdr.Caption := 'All Others ( ' + FormatFloat('0.##',
      StrToFloat(sgOthr.Cells[1, 0])) + ' acres)';
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      hasLuse[0] := true;
      copyContentsToGridNChk(PLRMObj.currentCatchment.primRdDrng, 0, 0,
        sgPrimRds);
    end;
    ...
    begin
      hasLuse[1] := true;
      copyContentsToGridNChk(PLRMObj.currentCatchment.secRdDrng, 0, 0,
        sgSecRds);
    end;
    ...
    sgOthr.Cells[2, 0] := FormatFloat('0.##',
      (PLRMObj.currentCatchment.othrPrcntImpv *
      PLRMObj.currentCatchment.othrArea / 100));

    lblPrrHdr.Caption := 'Primary Roads ( ' + FormatFloat('0.##', curAcVals[0])
      + ' acres)';
    lblSerHdr.Caption := 'Secondary Roads ( ' + FormatFloat('0.##', curAcVals[1]
      ) + ' acres)';
    lblSfrHdr.Caption := 'Single Family Residential ( ' +
      FormatFloat('0.##', curAcVals[2]) + ' acres)';
    lblMfrHdr.Caption := 'Multi-Family Residential ( ' +
      FormatFloat('0.##', curAcVals[3]) + ' acres)';
    lblCicHdr.Caption := 'CICU ( ' + FormatFloat('0.##', curAcVals[4]) +
      ' acres)';
    lblVegHdr.Caption := 'Vegetated Turf ( ' + FormatFloat('0.##', curAcVals[5])
      + ' acres)';
    lblOthHdr.Caption := 'All Others ( ' + FormatFloat('0.##',
      StrToFloat(sgOthr.Cells[1, 0])) + ' acres)';

    // 2014 if soils changed mid ran we dont want to overwrite ksat with previous values from user's xml so copy back calculated defaults
    // from the database
    if ((assigned(PLRMObj.currentCatchment.sfrDrng)) and
      (GSPLRM.PLRMObj.currentCatchment.hasChangedSoils = true)) then
    begin
      for I := 0 to High(sgs) do
      begin
        for J := 0 to 1 do
        ...
        sgSecRds.Cells[4, 2] := FormatFloat('0.##',
          StrToFloat(soilsInfData[0, 1]) * StrToFloat(kSatMultplrs[0][1]));
        sgPrimRds.Cells[4, 2] := FormatFloat('0.##',
          StrToFloat(soilsInfData[0, 1]) * StrToFloat(kSatMultplrs[0][0]));
      end;
    end;
...
begin
  ...
  curCatchArea := StrToFloat(PLRMObj.currentCatchment.swmmCatch.Data
    [UProject.SUBCATCH_AREA_INDEX]);

  for I := 0 to High(PLRMObj.currentCatchment.primRdSchms) do
    getHydSchemes(PLRMObj.currentCatchment.primRdSchms[I], I, HYDSCHMEXTS[I],
      'Roads', 'Prr');

  for I := 0 to High(PLRMObj.currentCatchment.secRdSchms) do
    getHydSchemes(PLRMObj.currentCatchment.secRdSchms[I], I, HYDSCHMEXTS[I],
      'Roads', 'Ser');

  for I := 0 to High(PLRMObj.currentCatchment.sfrSchms) - 1 do
    getHydSchemes(PLRMObj.currentCatchment.sfrSchms[I], I, HYDSCHMEXTS[I + 3],
      'Parcels', 'Sfr');

  for I := 0 to High(PLRMObj.currentCatchment.mfrSchms) - 1 do
    getHydSchemes(PLRMObj.currentCatchment.mfrSchms[I], I, HYDSCHMEXTS[I + 3],
      'Parcels', 'Mfr');

  for I := 0 to High(PLRMObj.currentCatchment.cicuSchms) - 1 do
    getHydSchemes(PLRMObj.currentCatchment.cicuSchms[I], I, HYDSCHMEXTS[I + 3],
      'Parcels', 'Cic');

  getHydSchemes(PLRMObj.currentCatchment.vegTSchms[0], 0, HYDSCHMEXTS[3],
    'Parcels', 'Vgt');
  getHydSchemes(PLRMObj.currentCatchment.othrSchms[0], 0, HYDSCHMEXTS[3],
    'Parcels', 'Othr');
  initFormContents(PLRMObj.currentCatchment);
...
procedure TPLRMDrainageConditions.getHydSchemes(var Schm: TPLRMHydPropsScheme;
  sType: Integer; ext: String; parcelOrRoad: String; luse: String);
var
  I: Integer;
...
procedure TPLRMDrainageConditions.checkAndSelectCbxItm(var cbx: TComboBox;
  scheme: TPLRMHydPropsScheme; schms: TStringList);
var
  tempInt: Integer;
begin
...
procedure TPLRMDrainageConditions.sgGenDrawCell(Sender: TObject;
...
procedure TPLRMDrainageConditions.sgGenParcelDrawCell(Sender: TObject;
  ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
begin
  GSUtils.sgGrayOnDrawCell3ColsOnly(Sender, ACol, ARow, Rect, State, 0, 1, 2);
end;

procedure TPLRMDrainageConditions.sgRoadGenSelectCell(Sender: TObject;
...
procedure TPLRMDrainageConditions.sgParcelGenSelectCell(Sender: TObject;
  ACol, ARow: Integer; var CanSelect: Boolean);
begin
...
procedure TPLRMDrainageConditions.sgGenSetEditText(Sender: TObject;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tempInfFootPrintArea := StrToFloat(sg.Cells[0, 0]) *
          StrToFloat(sg.Cells[1, 0]) / StrToFloat(prevGridValue);
        if ((ACol = 0) and (tempInfFootPrintArea < 0.01) and
          (tempInfFootPrintArea > 0)) then
        begin
          ShowMessage
            ('WARNING: This change will result in an infiltration facility with an area less than 0.01 acres.');
          // sg.Cells[ACol,ARow] := prevGridValue;
          Exit;
        end
      end;
      sg.Cells[0, sg.RowCount - 1] := FormatFloat('0.###', (100 - tempSum));
      // update last row of first column
      updateCalcs(sg);
    end;
...
procedure TPLRMDrainageConditions.sgVegDrawCell(Sender: TObject;
  ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
begin
  GSUtils.sgGrayOnDrawCell3ColsOnly(Sender, ACol, ARow, Rect, State, 0, 1, 2);
end;
...
begin
  ...
  begin
    sg.Cells[1, irow] := FormatFloat('0.##',
      (StrToFloat(sg.Cells[0, irow]) * area / 100));
    sg.Cells[2, irow] := FormatFloat('0.##',
      (StrToFloat(sg.Cells[1, irow]) * prcntImpv / 100));
  end;
end;

// used for road grids that need the first column to be editable but not the last cell in the first column
procedure TPLRMDrainageConditions.sgGenRdSelectCell(Sender: TObject;
  ACol, ARow: Integer; var CanSelect: Boolean);
var
  sg: TStringGrid;
begin
...
procedure TPLRMDrainageConditions.sgGenRdDrawCell(Sender: TObject;
  ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
begin
  GSUtils.sgGrayOnDrawCell3ColsOnly(Sender, ACol, ARow, Rect, State, 1, 1, 2);
...
procedure TPLRMDrainageConditions.loadHydPropsSchemeDb
  (var Schm: TPLRMHydPropsScheme; ext: String);
begin
  PLRMObj.loadHydPropsSchemeFromDb(ext, PLRMObj.currentCatchment.soilsInfData);
  Schm := PLRMObj.hydPropsSchemes.Objects[PLRMObj.curhydPropsSchmID]
    as TPLRMHydPropsScheme;
