...
type
  TPLRMScenComps = class(TForm)
    ...
    function getScenarioSummary(filePath:String; searchLineStr:String; rowOffSet:Integer):PLRMGridData;
  ...
  public
    { Public declarations }
    procedure genericExportGridToTxt(Sender: TObject; sg:TStringGrid; strFilter:String; fileName:String);overload;
    procedure genericExportGridToTxt(Sender: TObject; sg:TStringGrid; sg2:TStringGrid; strFilter:String; fileName:String);overload;
  end;
...
begin
  ...
  data2 := getScenarioSummary(defaultPrjDir + '\' + prjName + '\' + scnDirname + '\' + GSDETAILRPTFILENAME,searchStr,5);
  //data2 := getScenarioSummary(defaultPrjDir + '\' + prjName + '\' + scnDirname + '\' + 'swmm.prpt',searchStr,5);
  if data2 = nil then  exit;
  ...
  begin
    sgLoads.Cells[I+1,curRowIdx] := FormatFloat('0.0',strToFloat(data2[0,I]));
    sgAbslut.Cells[I+1,curRowIdx] := FormatFloat('0.0',strToFloat(data1[0,I]) - strToFloat(data2[0,I]));
    if strToFloat(data1[0,I]) = 0.0 then
      sgReltv.Cells[I+1,curRowIdx] := '0'
    else
      sgReltv.Cells[I+1,curRowIdx] := FormatFloat('0.0',100*(strToFloat(sgAbslut.Cells[I+1,curRowIdx])/strToFloat(data1[0,I]))) + '%';
  end;
...
begin
   genericExportGridToTxt(Sender,sgLoads,sgAbslut,'Scenario Comparison-Absolute(*.csv)', ' Scenario Comparison-Absolute');
end;

procedure TPLRMScenComps.btnExptRltvClick(Sender: TObject);
begin
    genericExportGridToTxt(Sender,sgLoads,sgRelTv,'Scenario Comparison-Relative(*.csv)', ' Scenario Comparison-Relative');
end;
...
begin
  ...
  if lbxScnRight.Items.Count = 0 then (Sender as Tbutton).enabled := false;
end;
...
begin
  ...
  begin
    filesList := getFilesInFolder(defaultPrjDir + '\' + prjName + '\' + S[I],'*.xml');
   // if not(fileExists(defaultPrjDir + '\' + prjName + '\' + S[I] + '\' + filesList[0])) then
   if (filesList.Count = 0) then
    begin
      ShowMessage('Scenario ' +  S[I] + ' at' + defaultPrjDir + '\' + prjName + '\' + S[I] + ' is invalid. Please delete the scenario folder and try again');
      exit;
    end;

    ScnNames.Add(getXMLRootChildTagValue('ScenName', defaultPrjDir + '\' + prjName + '\' + S[I] + '\' + filesList[0]) + '(' + S[I] + ')');
    filesList.Clear;
  end;
  cbxScenarios.Items :=  ScnNames;

  lbxScnLeft.Items := cbxScenarios.Items;
  lblPrjName.Caption := prjName + ' (with ' + intToStr(cbxScenarios.Items.Count) + ' scenarios)';
...
//    scnDirname :=tempStrLst[0];
    ...
    lbxScnLeft.Items := cbxScenarios.Items;     //added so that lbxScnleft items do not get deleted after addition of fxnality allowing baseline scn to be changed
...
begin
  ...
  begin
    ...
    begin
      ...
      sgAbslut.Cells[J+1,I] := FormatFloat('0.0',strToFloat(data1[0,J]) - strToFloat(data2[0,J]));
      if strToFloat(data1[0,J]) = 0.0 then
        sgReltv.Cells[J+1,I] := '0'
      else
        sgReltv.Cells[J+1,I] := FormatFloat('0.0',100*(strToFloat(sgAbslut.Cells[J+1,I])/strToFloat(data1[0,J]))) + '%';
    end;
  end;
end;
...
//  S := TStringList.Create();
  ...
  begin
    ...
    PrjNames.Add(getXMLRootChildTagValue('ProjectUserName', defaultPrjDir + '\' + S[I] + '\' + filesList[0]) + '(' + S[I] + ')');
    filesList.Clear;
  end;
  cbxProjects.Items :=  PrjNames;
end;

function TPLRMScenComps.getScenarioSummary(filePath:String; searchLineStr:String; rowOffSet:Integer):PLRMGridData;
var
  S:TStrings;
  tempInt:Integer;
  tempStr:String;
...
begin
  ...
  begin
    ShowMessage('scenario file not found. Please run the simulation to generate results for comparison');
    Result := nil;
    exit
  end;
  ...
  if tempInt = -1 then
  begin
    ShowMessage('A valid results file was not found at the specified location. Re-run PLRM and try again');
    Result := nil;
    exit;
  end;
...
var S : String;
begin
  if ((ARow=0) ) then begin //or (ARow = 0 ))then begin
