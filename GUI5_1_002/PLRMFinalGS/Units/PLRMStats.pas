...
function GetSWTStats(const tempNode: TPLRMNode; var perCap: Double)
  : PLRMGridDataDbl;
...
begin
  ...
  // 9-stTotalLoad)
  if GetStatsSelection(Stats, LinkObjName, LINKS, VarNumber, StatsIndex,
    PeriodIndex) then
  begin
  ...
  begin
    VarNumber := LINKQUAL + J;
    // LINKQUAL=9 is the first pollutant in Uglobals Link variables list
    if GetStatsSelection(Stats, LinkObjName, LINKS, VarNumber, StatsIndex,
      PeriodIndex) then
    begin
      GetStats(Stats, EventList, AnalysisResults);
...
begin
  ...
  AnnLoads := GetAveAnnualLoadsForLink(InLinkID);
  ...
  case tempNode.SWTType of
    ...
    2: // Infiltration
      begin
        ...
        if assigned(AnnLoads) then
          AnnLoads.Clear;
        AnnLoads := GetAveAnnualLoadsForLink(Tr2LinkID);
        // StringList of annual loads for Link
        for I := 0 to AnnLoads.Count - 1 do
  ...
  if assigned(AnnLoads) then
    AnnLoads.Clear;
  AnnLoads := GetAveAnnualLoadsForLink(ByLinkID);
  // StringList of annual loads for Link
  for I := 0 to AnnLoads.Count - 1 do
  ...
  if assigned(AnnLoads) then
    AnnLoads.Clear;
  AnnLoads := GetAveAnnualLoadsForLink(DsLinkID);
  // StringList of annual loads for Link
  for I := 0 to AnnLoads.Count - 1 do
...
var
  ...
  outLoads: TStringList;
  // String list containing pollutant loads for a outfall link
  tempSysLds: PLRMGridDataDbl;
  // volumes and loads summary for entire simulation
  I, tempInt: Integer;
begin
  OutLnkID := outfallName + '_UsCo';
  // Get annual loads for outfall link and transfer to SWTLoads array
  outLoads := GetAveAnnualLoadsForLink(OutLnkID);
  // StringList of annual loads for Link
  tempInt := outLoads.Count;
  SetLength(tempSysLds, tempInt, 1);
  for I := 0 to outLoads.Count - 1 do
    tempSysLds[I, 0] := StrToFloat(outLoads[I]);
  outLoads.Clear;
...
var
  OutLnkID: String; // outfall link name
  catchLoads: TStringList;
  // String list containing pollutant loads for a catchment
...
begin
  ...
  catchLoads := GetAveAnnualLoadsForLink(OutLnkID);
  // StringList of annual loads for Link
...
begin
  ...
  PLRMResults.numYrsSimulated :=
    Round((EndDateTime - StartDateTime) / 365.2422);
  ...
  begin
    ...
    begin
      ...
      if PLRMResults.outfallLoads = nil then
        SetLength(PLRMResults.outfallLoads, High(tempLoads) + 1,
