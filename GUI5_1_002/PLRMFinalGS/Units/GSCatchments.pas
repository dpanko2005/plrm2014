...
uses
 SysUtils, Windows, Messages, Classes, Controls, Forms, Dialogs, XMLIntf, msxmldom, XMLDoc,
  ...
  type TPLRMHydPropsScheme = class
  //flags
  isSet:boolean;
  ...
  description:String;
  category:String;
  stype:Integer; //sType -0 - to outlet, 1 - to infiltration, 2 - to pervious dispersion
  snowPackID:String;
  luse:String; //added to seperate schemes of the same stype and belonging to the same catchment but assigned to dif luses

  //data from _PLRMD6DrngXtsDetail form
  drngHydProps: PLRMGridData;   // Drainage area hydrologic properties grid values
  hydPropsHSC: PLRMGridData; // HSC hydrologic properties grid values for infiltration or pervious dispersion
 ...
 type SchemeArray = array[0..2] of TPLRMHydPropsScheme;
type TPLRMCatch = class
  //identification
  id :Integer;
  name:String;
  tempOutNodeID:String; //temporarily holds node name when read in from xml till actual node is linked
  outNode:TPLRMNode;
  mySWMMIndex :Integer;
  area :Double;
  slope :Double;
  ...
  ObjType:Integer; //Swmm assigned type used to retrieve obj in project lists
  ObjIndex:Integer; //SWmm assigned type index used to retrieve obj in project lists
  ...
  swmmCatchDefProps :PLRMGridData;//Table of default swmm properties [propName,propValue,units]
  ...
  sfrSchms: array[0..2] of TPLRMHydPropsScheme; //more than needed for uniformity and possible future extension
  mfrSchms: array[0..2] of  TPLRMHydPropsScheme; //more than needed for uniformity and possible future extension
  cicuSchms: array[0..2] of  TPLRMHydPropsScheme;//more than needed for uniformity and possible future extension
  vegTSchms: array[0..2] of  TPLRMHydPropsScheme; //more than needed for uniformity and possible future extension
  othrSchms: array[0..2] of  TPLRMHydPropsScheme;//more than needed for uniformity and possible future extension
  catchHydPropSchemes: array of array of  TPLRMHydPropsScheme;  //used to store all of the above arrays (see constructor) for easy processing
  ...
  function catchToXML(projectLuseNames:TStringList; ProjectLuseCodes:TStringList):IXMLNode;
  procedure xmlToCatch(iNode:IXMLNode; hydSchemes:TStringList; rcSchemes:TStringList);

  public
  procedure updateCurCatchProps(newName:String; physclProps:PLRMGridData; outGSNode:TPLRMNode);  // always use PLRMObj version so that PLRMObj catchment list is updated
  procedure copyHydSchemesToArray();
 end;

 procedure freeListofHydpropSchemes(var SchmList:TStringList);
 var
  swmmDefaultBlockXMLTags : array[0..5] of String = ('Options','Evaporation','Temperature','Aquifers','GroundWater','SnowPacks');
  luseXMLTags : array[0..3] of String = ('name','percentArea','percentImprv','area');
  soilsXMLTags : array[0..2] of String = ('name','percntArea','acres');
  soilsXMLTags2 : array[0..2] of String = ('ksat','ssh','smd');
  rdRiskCatXMLTags : array[0..2] of String = ('high','medium','low');
  rdShouldrCondsXMLTags : array[0..4] of String = ('highprcntUnimprovd','prcntProtOnly','prcntStabOnly','prcntStabAndProt','rdShouldrScore');
  rdReportCardXMLTags : array[0..1] of String = ('PollPotScore','SweepEffctvScore');
  runoffConcsXMLTags : array[0..5] of String = ('TSS','FSP','TP','SRP','TN','DIN');
  pollDelFactorXMLTags : array[0..1] of String = ('finesDisvd','particulates');
  bmpImplXMLTags : array[0..2] of String = ('noBMPs','srcCtrlCert','bmpCert');
  parcelAndRdMethXMLTags : array[0..6] of String = ('prcntArea','areaAc','impervArea','dcia','ksat','pervDepStor','imprvDepStor');
  drngSchmTypeXMLAttribTags :array[0..2] of string = ('outSchmID','infSchmID', 'dspSchmID');
implementation
uses
...
procedure TPLRMHydPropsScheme.readSchemeDb(schmExt:String; soilsInfData:PLRMGridData);
    var
  ...
  begin
    ...
    case stype of
        ...
        begin
          ...
          if(assigned(soilsInfData)) then
          begin
            hydPropsHSC[2,0] := FormatFloat('#0.000',StrToFloat(soilsInfData[0][1]));
            hydPropsHSC[2,1] := 'in/hr';
            hydPropsHSC[2,2] := FormatFloat('#0.000',StrToFloat(soilsInfData[0][1]));
          end
          else
  ...
  begin
    ...
    hydPropsGA[0,0] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ksat'].Attributes['default'];
    hydPropsGA[0,1] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ksat'].Attributes['units'];
    hydPropsGA[0,2] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ksat'].Text;

    hydPropsGA[1,0] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ssh'].Attributes['default'];
    hydPropsGA[1,1] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ssh'].Attributes['units'];
    hydPropsGA[1,2] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Ssh'].Text;

    hydPropsGA[2,0] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Smd'].Attributes['default'];
    hydPropsGA[2,1] := iNode.ChildNodes['GreenAmpt'].ChildNodes['Smd'].Attributes['units'];
    ...
    case stype of
        ...
        begin
          ...
          hydPropsHSC[0,0] := iNode.ChildNodes['UnitStorArea'].Attributes['default'];
          hydPropsHSC[0,1] := iNode.ChildNodes['UnitStorArea'].Attributes['units'];
        ...
        begin
          ...
          hydPropsHSC[3,0] := iNode.ChildNodes['PDAdepStor'].Attributes['default'];
...
var
  ...
  begin
    ...
    tempNodeLst4 := GSUtils.plrmGridDataToXML2('RoadShoulderConditions1',shoulderConds,rdShouldrTags,tempLst,tempLst);
    ...
    tempNodeLst5 := GSUtils.plrmGridDataToXML2('PPS',rdReportCardPPS,reportCardTagsPPS,tempLst,tempLst);
    ...
    tempNodeLst5 := GSUtils.plrmGridDataToXML2('SES',rdReportCardSES,reportCardTagsSES,tempLst,tempLst);
    ...
    tempNodeLst6 := GSUtils.plrmGridDataToXML2('RunoffConcentrations1',runoffConcs,runoffConcsTags,tempLst,tempLst);
    ...
    tempNodeLst7 := GSUtils.plrmGridDataToXML2('PollutantDeliveryFactors1',pollDelFactors,pollDelFactorTags,tempLst,tempLst);
  ...
  begin
    ...
    tempNode1 := iNode.ChildNodes['RoadConditions'].ChildNodes['RoadAbbrasiveAppl'];
    for I := 0 to High(cats) do
    begin
    ...
    shoulderConds := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['RoadShoulderConditions'],rdShouldrTags);
    pollDelFactors := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['PollutantDeliveryFactors'],pollDelFactorTags);
    rdReportCardPPS := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['PollutantPotentailScore'],reportCardTagsPPS);
    rdReportCardSES := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['SweeperEffectivenessScore'],reportCardTagsSES);
    runoffConcs := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['RunoffConcentrations'],runoffConcsTags);
 end;
...
begin
  ...
  begin
    ...
    //be referencing them
      ...
      cicuSchms[I] := nil;
      vegTSchms[I] := nil;
      othrSchms[I] := nil; //memory already free by another scheme pointing to same memeory
  end;
...
procedure TPLRMCatch.updateCurCatchProps(newName:String; physclProps:PLRMGridData; outGSNode:TPLRMNode);
begin
  name := newName;
  area :=   StrToFloat(physclProps[0,1]);
  width := Power(area,widthPower);
  if (width > maxFloLength) then width := maxFloLength;
...
function TPLRMCatch.catchToXML(projectLuseNames:TStringList; ProjectLuseCodes:TStringList):IXMLNode;
  var
   xmlDoc : IXMLDocument;
   iNode : IXMLNode;
   tempNode: IXMLNode;
  ...
  begin
    ...
    begin
      iNode.Attributes['outNode'] := outNode.userName; // must be the same as tempOutNodeID
    ...
    luseCodeList := lookUpCodeFrmName(landUseData,0,projectLuseNames,projectLuseCodes);
    //write all land uses info
    if (assigned(landUseData)) then
    begin
    tempNodeList := GSUtils.swmmInptFileLandUseToXML('LandUse',landUseData,luseTagList,0,luseCodeList);
    ...
    begin
    tempNodeList := GSUtils.swmmInptFileSoilsToXML('Soil',soilsMapUnitData,soilsTagList,0);
   ...
   //write road risk categories
    ...
    tempNodeList := GSUtils.plrmGridDataToXML('RoadRiskCat',rdRiskCats,rdRiskTagList,tempList);
    ...
    tempNodeList := GSUtils.plrmGridDataToXML('BmpImpl',bmpImpl,bmpImplTagList,tempList2);
...
//
    ...
    begin
      ...
      case I of
        0:
        begin
          if (primRdSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := primRdSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          if (primRdSchms[1] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := primRdSchms[1].id;
            tempHscParam[0] := strToFloat(primRdSchms[1].hydPropsHSC[0,2]) * strToFloat(primRdDrng[0,2])* CONVCUFT; //inf numbers in first row of grid at index 0
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
          if (primRdSchms[2] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := primRdSchms[2].id;
            tempHscParam[1] := strToFloat(primRdSchms[2].hydPropsHSC[0,2]) //* strToFloat(primRdDrng[1,2])* CONVCUFT; //dsp numbers in second row of grid at index 1
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := '-1';
        end;
        1:
        begin
          if (secRdSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := secRdSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          if (secRdSchms[1] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := secRdSchms[1].id;
            tempHscParam[0] := strToFloat(secRdSchms[1].hydPropsHSC[0,2]) * strToFloat(secRdDrng[0,2])* CONVCUFT; //inf numbers in first row of grid at index 0
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
          if (secRdSchms[2] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := secRdSchms[2].id;
            tempHscParam[1] := strToFloat(secRdSchms[2].hydPropsHSC[0,2]) //* strToFloat(secRdDrng[1,2])* CONVCUFT; //dsp numbers in second row of grid at index 1
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := '-1';
        end;
        ...
        2:
        begin
          if (sfrSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := sfrSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          if (sfrSchms[1] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := sfrSchms[1].id;
            tempHscParam[0] := strToFloat(sfrSchms[1].hydPropsHSC[0,2]) * strToFloat(sfrDrng[0,2])* CONVCUFT; //inf numbers in second row of grid at index 0
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
        ...
        3:
        begin
          if (mfrSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := mfrSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          if (mfrSchms[1] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := mfrSchms[1].id;
            tempHscParam[0] := strToFloat(mfrSchms[1].hydPropsHSC[0,2]) * strToFloat(mfrDrng[0,2])* CONVCUFT; //inf numbers in second row of grid at index 0
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
          tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := '-1';
        end;
        4:
        begin
          if (cicuSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := cicuSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          if (cicuSchms[1] <> nil) then
          begin
            tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := cicuSchms[1].id;
            tempHscParam[0] := strToFloat(cicuSchms[1].hydPropsHSC[0,2]) * strToFloat(cicuDrng[0,2])* CONVCUFT; //inf numbers in second row of grid at index 0
          end
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
          tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := '-1';
        end;
        5:
        begin
          if (vegTSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := vegTSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
        ...
        6:
        begin
          if (othrSchms[0] <> nil) then tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := othrSchms[0].id
          else tempNode2.Attributes[drngSchmTypeXMLAttribTags[0]] := '-1';
          tempNode2.Attributes[drngSchmTypeXMLAttribTags[1]] := '-1';
          tempNode2.Attributes[drngSchmTypeXMLAttribTags[2]] := '-1';
        end;
      ...
      //tempNodeList := GSUtils.plrmGridDataToXML('meth',tempDrnGridArr[I],parcelAndRdMethTagList,tempListDrngArr[I],tempListDrngArr[I]);
      tempNodeList := GSUtils.plrmGridDataToXML('meth',tempDrnGridArr[I],parcelAndRdMethTagList,tempTextListDrngArr[I],tempListDrngArr[I]);
      for J := 0 to tempNodeList.Count - 1 do
      begin
      //get area, calc width if area is not 0 and add it as an attribute
        tempArea := strToFloat(tempNodeList[J].Attributes['areaAc']) * CONVACFT;
        if tempArea <> 0 then
        begin
          ...
          tempNodeList[J].Attributes['width'] := FormatFloat('0.0',tempWidth);
        end;
          tempWidth := 0;
        ...
        begin
          ...
          tempNodeList[J].Attributes['hscArea'] := FormatFloat('0.000',tempArea);
          tempNodeList[J].Attributes['hscWidth'] := FormatFloat('0.000',tempWidth);
        end;
        tempWidth := 0;
  ...
  procedure TPLRMCatch.xmlToCatch(iNode:IXMLNode; hydSchemes:TStringList; rcSchemes:TStringList);
  var
   luseTagList:TStringList;
   soilsTagList:TStringList;
   rdRiskTagList:TStringList;
  ...
  begin
    ...
    for I := 0 to High(parcelAndRdMethXMLTags) do
      ...
      //read land uses info
      landUseData := xmlAttribToPlrmGridData(iNode.ChildNodes['LandUses'],luseTagList);
      for I := 0 to High(landUseData) do
        landUseNames.Add(landUseData[I,0]);
      //read soils info
      soilsMapUnitData := xmlToPLRMGridData(iNode.ChildNodes['Soils'],soilsTagList);
      ...
      begin
        rdRiskCats := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['RoadRiskCategories'],rdRiskTagList);
        if (iNode.ChildNodes['RoadRiskCategories'].HasAttribute('secSchmID')) then
          secSchmID := iNode.ChildNodes['RoadRiskCategories'].Attributes['secSchmID'];
        if (iNode.ChildNodes['RoadRiskCategories'].HasAttribute('primSchmID')) then
          primSchmID := iNode.ChildNodes['RoadRiskCategories'].Attributes['primSchmID'];
      end;
      if assigned(iNode.ChildNodes['BmpImplementation']) then
        bmpImpl := GSUtils.xmlAttribToPlrmGridData(iNode.ChildNodes['BmpImplementation'],bmpImplTagList);

      for I := 0 to High(frmsLuses) do
      begin
        tempNode := iNode.ChildNodes['ParcelAndRdMethXMLTags'].ChildNodes[frmsLuseTags[I]];
        tempDrnGridArr[I] := GSUtils.xmlAttribToPlrmGridData(tempNode,parcelAndRdMethTagList);
