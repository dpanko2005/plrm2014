...
uses
 SysUtils, Windows, Messages, Classes, Controls, Forms, Dialogs, XMLIntf, msxmldom, XMLDoc,
  StdCtrls, ComCtrls, Grids, GSUtils, GSTypes, Uproject, GSCatchments, GSNodes, GSPLRM;

 type TProjTree = Class
  public
    PID:TStringList; //project id string list with string lists of scenario IDs
    PrjNames:TStringList; //user supplied project names string list with string list of user scenario names
    ...
    procedure renameScen(prjID: String; oldUserScenName: String; newUserScenName: String);
...
begin
  ...
  if PID <> nil  then FreeStringListObjects(PID);
  if PrjNames <> nil  then FreeStringListObjects(PrjNames);
...
begin
      ...
      if userPrjName = tempStr then Result := PLRMTree.PID[tempInt];
end;

function TProjTree.getScenIDFromUserName(prjID:String; userScenName:String):String;
var
  scnID: String;
  tempSL : TStringList;
  prjIdx: Integer;
...
begin
  ...
  for I := 0 to tempSL.Count - 1 do
    begin
     ...
     if userScenName = scnName then Result := scnID;
    end;
    if Result = '' then ShowMessage('No ID found for ' + userScenName);
end;
...
var
  frmScnSL1, frmScnSL2, toScnSL1, toScnSL2 : TStringList; //scenario IDs and scenario names, respectively
  prjIdx: Integer;
  I : Integer;
begin
...
procedure TProjTree.renameScen(prjID: String; oldUserScenName: String; newUserScenName: String);
var
  scnID: String;
  prjIdx: Integer;
  scnIdx: Integer;
begin
 ...
 ((PLRMTree.PrjNames.Objects[prjIdx]) as TStringList)[scnIdx] := newUserScenName;
end;
 ...
 function TProjTree.checkScnNameUsed(newPrjName:String; newScenName:String):Boolean;
 var
  tempInt:Integer;
 begin
    tempInt := PLRMTree.PrjNames.IndexOf(newPrjName);
    tempInt := ((PLRMTree.PrjNames.Objects[tempInt]) as TStringList).IndexOf(newScenName);
    if tempInt <> -1 then
      Result := true
    else
      Result := false;
