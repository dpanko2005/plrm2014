...
const
  ...
  PropNames: array[0..MAX_GW_PROPS] of String =
    ('Aquifer Name',
     ...
     'B2 Exponent',
     'A3 Coefficient',
     'Surface Water Height (Hsw)',
     'Channel Bottom Height (Hcb)',
     'Bottom Elevation',
     'Water Table Elevation',
     'Unsat. Zone Moisture',
     'Custom GW Flow Equation');

  DefaultProps: array[0..MAX_GW_PROPS] of String =
    ('', '*', '0', '0', '0', '0', '0', '0', '0', '',
    '', '', '', 'No');
  ...
  GroundWaterHint: array[0..MAX_GW_PROPS] of String =
    ('Name of aquifer object that lies below subcatchment ' +
     ...
     'Height of surface water above aquifer bottom (ft or m) ' +
     '(0 uses height from flow routing).',
     ...
     '(Leave blank to use Aquifer value).',
     'Click to supply a custom equation for GW flow.'
     );
...
begin
  ...
  // Set the attributes of the Custom Equation property
  I := MAX_GW_PROPS;
  GwaterProps[I].Name := PropNames[I];
...
procedure TGroundWaterForm.ButtonClick(Sender: TObject; Index: Integer;
      var S: String; var Modified: Boolean);
//-----------------------------------------------------------------------------
//  OnClick handler for the custom GW flow equation property
//-----------------------------------------------------------------------------
var
...
begin
  ...
  for K := 0 to High(DefaultProps) do PropList.Add(DefaultProps[K]);
  CustomEqn := '';
  ...
  begin
    ...
    CustomEqn := S.GwFlowEqn;
    if Length(CustomEqn) > 0 then PropList[GW_EQN_INDEX] := 'Yes'
    else PropList[GW_EQN_INDEX] := 'No';
  end;
  HasChanged := False;
end;
...
begin
  ...
  begin
    ...
    // Note: the last property in the editor's list is not part of the
    //       Groundwater string list
    if Length(Trim(PropList[0])) > 0
    then for K := 0 to MAX_GW_PROPS-1 do S.Groundwater.Add(PropList[K]);
    S.GwFlowEqn := CustomEqn;
...
begin
  ...
  for K := 0 to High(DefaultProps) do PropList.Add('*');
...
var
  K: Integer;
begin
  for K := 0 to MAX_GW_PROPS-1 do GWData[K] := PropList[K];
end;
