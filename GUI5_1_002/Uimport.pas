...
const
  ...
  SectionWords : array[0..52] of PChar =
    ('[TITLE',                    //0
     '[OPTION',                   //1
     '[RAINGAGE',                 //2
     '[HYDROGRAPH',               //3
...
var
  ...
  I  : Integer;
begin
  if nToks < MAXAQUIFERPROPS+1 then Result := ErrMsg(ITEMS_ERR, '')
  else begin
    ID := TokList[0];
    A := TAquifer.Create;
    Uutils.CopyStringArray(Project.DefProp[AQUIFER].Data, A.Data);
    Project.Lists[AQUIFER].AddObject(ID, A);
    Project.HasItems[AQUIFER] := True;
    for I := 0 to MAXAQUIFERPROPS do A.Data[I] := TokList[I+1];
    Result := 0;
  end;
end;
...
begin
  ...
  begin
    S.GwFlowEqn := Trim(AnsiRightStr(Line, Length(Line)-Length(TokList[0])));
    Result := 0;
  end;
end;
...
begin
  ...
  then Result := ErrMsg(ITEMS_ERR, '')
    ...
    begin
      ...
      // Seepage rate
      if Ntoks > N+2 then aNode.Data[STORAGE_SEEPAGE_INDEX] := TokList[N+2];
      // Seepage rate for deprecated Green-Ampt infil. parameters
      if Ntoks > N+3 then  aNode.Data[STORAGE_SEEPAGE_INDEX] := TokList[N+3];
    end;
    aNode.Data[COMMENT_INDEX ] := Comment;
      ...
      CONDUIT:
        begin
          ...
          begin
            ...
            aLink.Data[CONDUIT_GEOM1_INDEX] := 'N/A';
            Result := 0;
          end
          else begin
...
//-----------------------------------------------------------------------------
var
  S     : array[1..7] of String;                                                //(5.0.014 - LR)
  Inflow: String;
  I     : Integer;
  aNode : TNode;
begin
...
begin
  ...
  then Result := ErrMsg(ITEMS_ERR, '')
    ...
    begin
      ...
      aPattern.Comment := Comment;                                             //(5.0.022 - LR)
...
begin
  ...
  case Index of
    ...
    begin
      ...
      with MapForm.Map.Dimensions do                                            //(5.0.012 - LR)
      begin                                                                     //(5.0.012 - LR)
        if Units = muDegrees then Digits := MAXDEGDIGITS                        //(5.0.012 - LR)
        else Digits := Umap.DefMapDimensions.Digits;                            //(5.0.012 - LR)
      end;                                                                      //(5.0.012 - LR)
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Section = 0) and (Ntoks > 0) then Err := ReadTitleData(Line)
