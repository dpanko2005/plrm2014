...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons, Contnrs, Uglobals, Uproject;
...
begin
  ...
  if I >= 0 then
  begin
    MessageDlg('Link ' + LinksListBox.Items[I] +
               ' either does not exist or is not connected' +
               ' to the preceding link.', mtError, [mbOK], 0);
    Exit;
  end;
  Hide;
  CreatePlot;
...
begin
  ...
  begin
    MessageDlg(MSG_START_NODE_NOT_EXIST, mtError, [mbOK], 0);
    exit;
  end;
  StartNode := Project.GetNode(I, J);
  if not Project.FindNode(EndNodeEdit.Text, I, J) then
  begin
    MessageDlg(MSG_END_NODE_NOT_EXIST, mtError, [mbOK], 0);
    exit;
  end;
  ...
  try
    ...
    then MessageDlg(MSG_NO_VALID_PATH, mtError, [mbOK], 0)
