...
//Graph series options
  ...
  TChartOptionsDlg = class(TForm)
    ...
    procedure XautoClick(Sender: TObject);
  private
...
procedure TChartOptionsDlg.XautoClick(Sender: TObject);
begin
  if Xauto.Checked then
  begin
    Xmin.Clear;
    Xmax.Clear;
    Xinc.Clear;
  end;
  Xmin.Enabled := not Xauto.Checked;
  Xmax.Enabled := Xmin.Enabled;
  Xinc.Enabled := Xmin.Enabled;
  XminLabel.Enabled := Xmin.Enabled;
  XmaxLabel.Enabled := Xmax.Enabled;
  XincrementLabel.Enabled := Xinc.Enabled;
end;

procedure TChartOptionsDlg.XFontBtnClick(Sender: TObject);
begin
  with FontDialog1 do
  begin
...
begin
  if (Sender is TComboBox) then
    ...
    begin
      GetSeriesOptions(SeriesIndex);  {Store options for current series}
...
begin
  ...
  { Bottom Axis }
    ...
    begin
      ...
      begin
        ...
        with BottomAxis do
        begin
          AxisInfo[1].AutoScale := Automatic;
  ...
  { Left Axis }
    ...
    begin
      ...
      with LeftAxis do
      begin
        AxisInfo[2].AutoScale := Automatic;
        if not Automatic then
        begin
  ...
  { Right Axis }
    ...
    begin
      ...
      with RightAxis do
      begin
        AxisInfo[3].AutoScale := Automatic;
        if not Automatic then
        begin
...
begin
  ...
  AxisInfo[AxisIndex].DataMin := XDataMinLabel.Caption;
  AxisInfo[AxisIndex].DataMax := XDataMaxLabel.Caption;
  AxisInfo[AxisIndex].AxisMin := XMin.Text;
  AxisInfo[AxisIndex].AxisMax := XMax.Text;
  AxisInfo[AxisIndex].AxisInc := XInc.Text;
  AxisInfo[AxisIndex].AutoScale := Xauto.Checked;
...
begin
  ...
  Xauto.Checked := AxisInfo[AxisIndex].AutoScale;
...
begin
  with theAxis do
  begin
    Automatic := theAxisInfo.AutoScale;
    ...
    if (theAxis = theChart.BottomAxis) and IsDateTime then
      ...
      SetAxisScaling(theAxis, theAxisInfo.AxisMin,
        theAxisInfo.AxisMax, theAxisInfo.AxisInc)
...
begin
  ...
  begin
    ...
    Val(Smin,v,code);
    if (code = 0) then Minimum := v
    else AutomaticMinimum := True;
    AutomaticMaximum := False;
    Val(Smax,v,code);
    if (code = 0) then Maximum := v
    else AutomaticMaximum := True;
    if AutomaticMinimum and AutomaticMaximum then Automatic := True
    else
    begin
      Val(Sinc,v,code);
      if (code = 0) then Increment := v
      else Increment := 0;
    end;
  end;
end;
