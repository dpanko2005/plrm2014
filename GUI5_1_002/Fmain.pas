...
uses
  ...
  GSTypes, GSInifile, // PLRM addition
...
type
  TMainForm = class(TForm)
    ...
    btnRecRngRprt: TToolButton;
    btnGISTool: TToolButton;
    btnGISCatchment: TToolButton;
    ...
    procedure btnViewRptClick(Sender: TObject);
    procedure btnViewRptDetailedClick(Sender: TObject);
    procedure btnRecRngRprtClick(Sender: TObject);
    procedure btnGISToolClick(Sender: TObject);
    procedure btnGISCatchmentClick(Sender: TObject); // PLRM 2014 Addition
...
uses
  ...
  _PLRM1ProjNscenManger, GSPLRM, GSUtils, _PLRMstats, _PLRMD7GISTool,
  _PLRMD7bGISTool, PLRMStats,
...
procedure TMainForm.btnGISCatchmentClick(Sender: TObject);
begin
  // -----------------------------------------------------------------------------
  // Shows GIS Catchment selection dialog which allows user to select and create
  // a catchment from previously created GIS xml file
  // -----------------------------------------------------------------------------
  begin
    // Create GIS Catchment selection form
    PLRMGISCatchDlg := TPLRMGISCatchDlg.Create(Self);
    with PLRMGISCatchDlg do
      try
        Caption := PLRM7bGIS_TITLE;
        PLRMGISCatchDlg.Show;
      finally
      end;
  end;
end;

procedure TMainForm.btnGISToolClick(Sender: TObject);
begin
  // -----------------------------------------------------------------------------
  ...
  begin
    ...
    // Check if Status Report form open and close it
    if FormExists('StatusForm') then
      StatusForm.Close();
...
// PLRM Addition
procedure TMainForm.btnRecRngRprtClick(Sender: TObject);
begin
...
begin
  // plrm 2014
  SaveIniFile(); // save plrm specific settings
...
procedure TMainForm.btnViewRptDetailedClick(Sender: TObject);
// -----------------------------------------------------------------------------
// Displays a run's Status Report when Report|Status is selected.
// -----------------------------------------------------------------------------
begin
...
begin
  Application.HelpKeyword(TXT_HOW_DO_I);
end;
