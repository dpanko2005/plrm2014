...
uses
  Dtblplot, Fmain, Uedit;

const
  MAXITEMS = 100;

////  Text labels modified to include Control Curve type. (LR - 9/5/05)  ////
////  Shape Curve type added & labels re-arranged. (5.0.010 - LR)  ////
  CurveTypeLabels: array[CONTROLCURVE..TIDALCURVE] of String =
    ('Control', 'Diversion', 'Pump', 'Rating', 'Shape', 'Storage', 'Tidal');

  PumpTypeLabels: array[0..3] of String =
    ('TYPE1', 'TYPE2', 'TYPE3', 'TYPE4');

////  Text labels modified. (LR - 9/5/05)  ////
////  Text labels re-arranged (5.0.010 - LR) ////
  Xlabel: array[CONTROLCURVE..TIDALCURVE] of String =
    ('Controller', 'Inflow', '', 'Head', 'Depth /', 'Depth', 'Hour');

  Xpump: array[0..3] of String =
    ('Volume', 'Depth', 'Head', 'Depth');

////  Text labels modified. (LR - 9/5/05)  ////
////  Text labels re-arranged. (5.0.010 - LR)  ////
  Xunits: array[CONTROLCURVE..TIDALCURVE, 0..1] of String =
    (('Value','Value'), ('',''), ('',''), ('(ft)','(m)'),
     ('Full Depth','Full Depth'), ('(ft)', '(m)'),  ('of Day', 'of Day'));

  XPumpUnits: array[0..3, 0..1] of String =
    (('(ft3)','(m3)'), ('(ft)','(m)'), ('(ft)','(m)'), ('(ft)','(m)'));

////  Text labels modified. (LR - 9/5/05)  ////
////  Text labels re-arranged. (5.0.010 - LR)  ////
  Ylabel: array[CONTROLCURVE..TIDALCURVE] of String =
    ('Control', 'Outflow', 'Flow', 'Outflow', 'Width /', 'Area', 'Stage');

////  Text labels modified. (LR - 9/5/05)  ////
////  Text labels re-arranged. (5.0.010 - LR)  ////
  Yunits: array[CONTROLCURVE..TIDALCURVE, 0..1] of String =
   (('Setting','Setting'), ('',''), ('',''), ('',''),
...
begin
  ...
  if Length(S) = 0 then
  begin
    MessageDlg(MSG_NO_ID, mtError, [mbOK], 0);
    CurveName.SetFocus;
    Result := False;
    Exit;
  end;
  ...
  if DupID then
  begin
    MessageDlg(MSG_DUPLICATE_ID, mtError, [mbOK], 0);
    CurveName.SetFocus;
    Result := False;
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if MessageDlg(S + MSG_OUT_OF_ORDER, mtConfirmation,
...
var
  UseStairs: Boolean;
  SwitchXY:  Boolean;                                                //Added (LR - 7/5/06)
  Title: String;
begin
  // Type 1 and Type 2 pump curves use stair-step display
  // Type 3 pump curve has X & Y axes switched                       //Added (LR - 7/5/06)
  SwitchXY  := False;
  UseStairs := False;
  if CurveType = PUMPCURVE then
  begin
    ...
    then UseStairs := True;
    if CurveTypeCombo.ItemIndex = 2 then SwitchXY := True;           //Added (LR - 7/5/06)
  end;

  // Create a form to plot the curve on
  with TTablePlotForm.Create(self) do
  try
    ...
    else if CurveType = SHAPECURVE                                     //(5.0.010 - LR)
    then PlotShapeXsect(GridEdit.Grid, Title)                          //(5.0.010 - LR)
    else PlotCurveData(GridEdit.Grid, Title, UseStairs, SwitchXY);     //Modified (LR - 7/5/06)
    ShowModal;
  finally
    Free;
  end;
