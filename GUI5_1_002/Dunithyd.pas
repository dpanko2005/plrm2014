...
begin
  ...
  begin
    DefaultColWidth := ((ClientWidth - 120) div 3) - 1;
    ColWidths[0] := 130;
    //DefaultRowHeight := (ClientHeight div 4) - 1;
...
begin
  ...
  if not Modified then Modified := IAGridEdit.Modified;                         //(5.0.010 - LR)
...
begin
  ...
  if Length(S) = 0 then
  begin
    MessageDlg(MSG_NO_RAINGAGE, mtError, [mbOK], 0);
    RGname.SetFocus;
    Result := False;
    Exit;
  end;
