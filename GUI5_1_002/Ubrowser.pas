...
begin
  ...
  begin
    if MessageDlg(TXT_DELETE_OBJECT,mtConfirmation,[mbYes,mbNo],0) = mrNo
    then Exit;
  end;
...
begin
  case Index of
    ...
    28: Result := CONTROLCURVE;
    29: Result := DIVERSIONCURVE;
    30: Result := PUMPCURVE;
    31: Result := RATINGCURVE;
    32: Result := SHAPECURVE;
    33: Result := STORAGECURVE;
    34: Result := TIDALCURVE;

    35: Result := TIMESERIES;
    36: Result := PATTERN;
    37: Result := MAPLABEL;
...
begin
  case ObjType of
    ...
    CONTROLCURVE:   Result := 28;
    DIVERSIONCURVE: Result := 29;
    PUMPCURVE:      Result := 30;
    RATINGCURVE:    Result := 31;
    SHAPECURVE:     Result := 32;
    STORAGECURVE:   Result := 33;
    TIDALCURVE:     Result := 34;

    TIMESERIES:   Result := 35;
    PATTERN:      Result := 36;
    MAPLABEL:     Result := 37;
...
begin
  ...
  begin
    Clear;
    for I := 0 to SUBCATCHOUTVAR1 - 1 do Items.Add(SubcatchVariable[I].Name);
...
var
  ...
  N: Integer;
begin
  N := Uglobals.Npolluts - 1;
  with MainForm.SubcatchViewBox do
  begin
    for I := SUBCATCHOUTVAR1 to SUBCATCHQUAL-1 do
  ...
  with MainForm.NodeViewBox do
  begin
    for I := NODEOUTVAR1 to NODEQUAL-1 do
  ...
  with MainForm.LinkViewBox do
  begin
    for I := LINKOUTVAR1 to LINKQUAL-1 do
...
//// Additional properties added. (LR - 9/5/05) ////
