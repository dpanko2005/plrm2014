...
{                    Author:  L. Rossman                            }
{                                                                   }
{   Stay-on-top form unit used to define the contents of graphical  }
{   and tabular reports. It consists of a Notebook control with     }
{   three pages; one for time series plots and tables, one for      }
{   profile plots, and  one for scatter plots. When the user        }
...
uses
  ...
  Spin, NumEdit, Uglobals, Uproject;
...
begin
  ...
  begin
    ...
    begin
      MessageDlg(MSG_TOO_FEW_ITEMS, mtError, [mbOK], 0);
      Exit;
    end;
  end
  ...
  begin
    ...
    begin
      MessageDlg(MSG_NO_ITEMS, mtError, [mbOK], 0);
      Exit;
    end
    ...
    begin
      MessageDlg(MSG_TOO_MANY_ITEMS, mtError, [mbOK], 0);
      Exit;
    end;
    ...
    begin
      MessageDlg(MSG_NO_VARIABLE, mtError, [mbOK], 0);
      Exit;
    end;
  end;
  ...
  then
  begin
    MessageDlg(MSG_INVALID_DATES, mtError, [mbOK], 0);
    Exit;
  end;
...
begin
  ...
  begin
    MessageDlg(MSG_NO_RESULTS_AVAILABLE, mtInformation, [mbOK], 0);
    Exit;
  end;
  ...
  0: if not Project.IsSubcatch(CurrentList) then
     begin
       MessageDlg(MSG_NO_SUBCATCH_SELECTED, mtError, [mbOK], 0);
       Exit;
     end;
  1: if not Project.IsNode(CurrentList) then
     begin
       MessageDlg(MSG_NO_NODE_SELECTED, mtError, [mbOK], 0);
       Exit;
     end;
  2: if not Project.IsLink(CurrentList) then
     begin
       MessageDlg(MSG_NO_LINK_SELECTED, mtError, [mbOK], 0);
       Exit;
     end;
  end;
