...
uses
  ...
  Dialogs, StdCtrls, Buttons, FileCtrl, Uglobals;
...
begin
  ...
  if Length(S1) = 0 then
  begin
    MessageDlg('Tool Name cannot be empty.', mtWarning, [mbOK], 0);
    ToolNameEdit.SetFocus;
  end
  else if Length(S2) = 0 then
  begin
    MessageDlg('Program name cannot be empty.', mtWarning, [mbOK], 0);
...
begin
  ...
  if I >= 0 then
    ToolDirEdit.Text := ToolParamsEdit.Text + Macros[I] + ' ';
end;
